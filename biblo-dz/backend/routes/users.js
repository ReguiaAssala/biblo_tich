...
router.get('/profile', auth, async (req, res) => {
  try {
    ...
    if (!r.length) return res.status(404).json({ message:'غير موجود' });
    res.json(r[0]);
...
router.put('/profile', auth, async (req, res) => {
  try {
    const { nom, prenom, universite, wilaya, annee_etude } = req.body;
...
router.get('/stats', auth, async (req, res) => {
  try {
    ...
    res.json({ uploads:up.c, downloads:dl.c, favorites:fv.c, ratings:nt.c });
  } catch(e) { res.status(500).json({ message:e.message }); }
});
...
router.get('/', auth, isAdmin, async (req, res) => {
  try {
    ...
    const p=[];
    if (search){ sql+=' AND (nom LIKE ? OR prenom LIKE ? OR email LIKE ?)'; const q=`%${search}%`; p.push(q,q,q); }
...
router.put('/:id/role', auth, isAdmin, async (req, res) => {
  try {
    const { role } = req.body;
    if (!['etudiant','moderateur','admin'].includes(role)) return res.status(400).json({ message:'دور غير صالح' });
