...
router.get('/documents', async (req, res) => {
  ...
  try 
  {
    ...
    const {
       q, specialite, type, niveau, sort='recent'
       , page=1, limit=12
    ...
    if (!q || q.trim().length < 2)
       ...
       .json(
        {
         message:'عبارة البحث يجب أن تكون حرفين على الأقل'
         }
        );
    ...
    res.json(
      ...
      {
       results:rows, total, page:parseInt(page),
        pages:Math.ceil(total/parseInt(limit)), query:q }
      );
  }
...
router.get('/', async (req, res) => {
  ...
  
  {
    ...
    .json(
      {
       message:'عبارة البحث قصيرة جداً' 
      }
    );
    const like = `%${q.trim()}%`;
